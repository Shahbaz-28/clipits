...
-- Campaigns policies
CREATE POLICY "Anyone can view active campaigns" ON campaigns FOR SELECT USING (
  status = 'active'
);
CREATE POLICY "Creators and admins can create campaigns" ON campaigns FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('creator', 'admin'))
);
CREATE POLICY "Campaign creators can update own campaigns" ON campaigns FOR UPDATE USING (
  created_by = auth.uid()
);
