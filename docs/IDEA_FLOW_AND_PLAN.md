# Rippl s — Complete Idea, Flow & Build Plan
...
**Rippl s** is a **Content Reward platform**: creators/brands run campaigns; participants post content on **Instagram**, submit the link, get approved, and **earn per view** (CPM). We manage campaigns, submissions, approval, and payouts. **Initial scope: Instagram only.**
...
| 6 | Submit in Rippl s: paste Instagram post/Reel link (+ optional media) | Submit form; stored in `submissions` with `platform: 'instagram'`. |
