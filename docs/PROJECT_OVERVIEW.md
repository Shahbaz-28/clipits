# Rippl s — Project Overview & Documentation
...
**Rippl s** is a **Content Reward platform** — similar in concept to [Whop Content Rewards](https://help.whop.com/en/articles/11429379-how-to-join-a-whop-content-reward). We connect:
...
- **Verification:** Optional **Instagram bio verification** (paste code in bio → verify in Rippl s) to prove ownership of the account.
...
5. **Submit in Rippl s** — User pastes the **Instagram post/Reel link** (and optionally uploads the media file). Submission is stored with `platform: "instagram"`.
6. **(Optional) Verify Instagram account** — To prove they own the account: Rippl s shows a verification code → user puts it in their **Instagram bio** → user clicks Verify in Rippl s. We confirm and mark the account as verified (or skip this in v1). See §1.5.1 for the full logic.
...
1. **User starts verification**  
   - User is on Rippl s (e.g. on “Submit” or “Settings → Linked accounts”).  
...
2. **We generate and show the code**  
   - Backend (Supabase Edge Function or DB + RPC) generates a **unique verification code** (e.g. 6–8 alphanumeric: `Rippl -A1B2C3`).  
...
3. **User adds the code to Instagram**  
   ...
   - User pastes the code (e.g. `Rippl -A1B2C3`) into the bio and saves.  
   - User returns to Rippl s and clicks **Verify**.
...
| 3 | User | Puts code in Instagram bio, saves, comes back to Rippl s, clicks Verify. |
...
- **From submission time.** As soon as the participant submits the post link (and optional media) in Rippl s, we consider that the “start” of tracking. Whop states: *“Only views after you submit count towards payout. Submit as soon as you post to get paid for all of your post’s views.”*
...
#### Recommended approach for Rippl s
